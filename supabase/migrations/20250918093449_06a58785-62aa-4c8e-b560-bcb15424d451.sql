-- Fix company financial data security vulnerabilities
...
-- Also enhance user_companies policies to prevent unauthorized relationship creation
...
WITH CHECK (
  ...
  -- And either:
  -- 1. They're creating their first admin relationship (company creator)
  -- 2. OR they're being added by an existing admin of that company
  AND (
    -- Case 1: No existing relationships for this company (new company)
    NOT EXISTS (SELECT 1 FROM public.user_companies WHERE company_id = NEW.company_id)
    OR
    -- Case 2: An admin of the target company is adding them
    EXISTS (
      SELECT 1 FROM public.user_companies uc
      WHERE uc.company_id = NEW.company_id
      AND uc.role = 'admin'
      -- This would need to be implemented in application logic for invitations
    )
  )
);